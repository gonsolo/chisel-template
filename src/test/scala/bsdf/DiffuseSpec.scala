...
class DiffuseSpec extends AnyFlatSpec with ChiselScalatestTester {
  ...
  it should "return inv pi" in {
    test(new Diffuse).withAnnotations(Seq(WriteVcdAnnotation)) { diffuse =>
      ...
      val testValues = for { x <- 1 to 1 } yield x.toFloat / 3.0f
      ...
      //diffuse.output.expectDequeueNow((new DiffuseOutputBundle).Lit(_.out.values(0) -> floatToIntBits(1.0f / 3.0f / Pi.toFloat).S))
      diffuse.output.expectDequeueNow(resultSeq(0))
    }
  }
}
